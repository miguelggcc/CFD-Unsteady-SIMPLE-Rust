...
impl<'p> Plot<'p> {
    ...
    pub fn pcolormesh(&mut self, x: &[f64], y: &[f64], z: &[f64], cmap: &str, title: &str) {
        ...
        subplots.set_item(self.py, "figsize", (8, 4)).unwrap();
        ...
        dict.set_item(self.py, "vmax", 1.3).unwrap();
        ...
        //fig.call_method(self.py, "tight_layout", NoArgs, None).unwrap();
    ...
    pub fn update_pcolormesh(&mut self, z: &[f64], x_len: usize, y_len: usize) {
...
impl<'p> Animation<'p> {
    pub fn new(env: &'p Env, plot: Plot<'p>, fps: u32) -> Animation<'p> {
        ...
        dict.set_item(py, "extra_args", vec!["-vcodec", "h264"])
            .unwrap();
